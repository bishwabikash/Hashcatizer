...
Two of them have since been built (see below). Until they are merged upstream,
the runtime message above stays as it is: it describes what a released hashcat
can do, which is what a user running `hashcat` from their package manager will
actually get.
...
### 2. `$PEM$2` verbose form — smallest possible PR — **implemented**
